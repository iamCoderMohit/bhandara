...
function EditDetails({setEdit}: {setEdit: React.Dispatch<boolean>}) {
  ...
  return (
    <div className="w-[30%] h-fit absolute -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2  bg-gray-800 z-10 rounded-md p-5">
        {}// show error msg here
        //when user successrully saves their infor redirect them to profile page
        // without showing edit components
      ...
      <Button text="Save details" handler={handleEdit} loading={loading} />    
      </div>
    </div>
  );
}
